...
The cover image can be either uploaded from device or generated by AI:

1. **Upload**: The user uploads an image file via a dedicated upload endpoint (`POST /api/uploads`), which stores the file on disk and returns the public URL (`GET /api/uploads/{filename}`)
2. **AI Generation** (Phase 3): The AI generates an image and the result is stored the same way
3. The returned URL is saved in `CoverImageUrl` when creating or updating the event
